  ...
  template: `
    <div class="page-container">
      ...
      <div *ngIf="detail" class="detail-content">
        <h1>Purchase Order #{{ detail.purchaseOrderId }}</h1>
        <div class="detail-info">
          <div class="detail-field"><span class="label">Supplier:</span><span class="value">{{ detail.supplierName }}</span></div>
          <div class="detail-field"><span class="label">Order Date:</span><span class="value">{{ detail.orderDate | date:'mediumDate' }}</span></div>
        </div>
        ...
        <table class="lines-table" *ngIf="detail.lines?.length">
          ...
          <tbody>
            <tr *ngFor="let line of detail.lines">
              ...
              <td>{{ line.orderedOuters }}</td>
              <td>{{ line.expectedUnitPricePerOuter | currency }}</td>
              <td>{{ line.orderedOuters * line.expectedUnitPricePerOuter | currency }}</td>
            </tr>
          </tbody>
        </table>
        <p *ngIf="!detail.lines?.length" class="empty">No purchase order lines.</p>
      </div>
    </div>
  `,
  styles: [`
...
export class PurchaseOrderDetailComponent implements OnInit {
  detail: PurchaseOrderDetail | null = null;
  loading = true;
  ...
  loadDetail(id: number): void {
    this.loading = true;
    this.apiService.getDetail<PurchaseOrderDetail>('purchaseorders', id).subscribe({
